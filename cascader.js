...
(function ($) {
    ...
    ZdCascader.METHODS = ['reload', 'destroy', 'select', 'close', 'open'];
    ...
    ZdCascader.prototype._nodeClick = function (event) {
        ...
        if (!data.children) {
            ...
            $that.prepend($(`<span class="${this.CLASS.checkClass.nodeSelectedIcon}">${this.options.iconSelected}</span>`));
            this.$el.data('bindData', data);
            this.$el.data('bindPathData', allPathData);
            if (this.options.onChange && typeof this.options.onChange === "function")
                this.options.onChange(this, data, allPathData);
            event.currentTarget ?? event.stopPropagation();
        } else
    ...
    ZdCascader.prototype._loadChildren = function ($parentNode) {
        ...
        $.each(data.children, (i, m) => {
            var $li = $(`<li class="${this.CLASS.menuNode}" data-id="${m.id}"></li>`);
            var $label = $(`<span class="${this.CLASS.menuNodeLabel}">${m.label}</span>`);
            var $icon = this.options.iconPostfix;
            ...
            if (m.children && m.children.length > 0) $li.append($icon);
                ...
                $li.addClass(this.CLASS.checkClass.menuNodeSelected).prepend($(
                    `<span class="${this.CLASS.checkClass.nodeSelectedIcon}">${this.options.iconSelected}</span>`));
            }
            $li.appendTo($ul);
        });
    }
    ...
    ZdCascader.prototype.getLabelList = function () {
        ...
        function traverse(node, path, valuePath) {
            if (!node.children) { // only add leaf nodes to the datas array
                datas.push({
                    label: path.join(' / '),
                    labels: path,
                    value: valuePath,
            ...
            if (node.children) {
                node.children.forEach(function (child) {
                    traverse(child, path.concat([child.label]), valuePath.concat([child.value]));
                });
            }
        }
        this.options.data.forEach(function (node) {
            traverse(node, [node.label], [node.value]);
        });
    ...
    ZdCascader.prototype.search = function (keyword) {
        ...
        var data = this.labelList.filter(function (item) {
            ...
            return b.label.toLowerCase().indexOf(keyword) - a.label.toLowerCase().indexOf(keyword);
        }).slice(0, 10);
    ...
    ZdCascader.prototype.reload = function (data, search) {
        ...
        $.each(data, (i, m) => {
            ...
            if (m.children && m.children.length > 0) $li.append($icon);
                ...
                this.$dropdownWrap.find('.' + this.CLASS.checkClass.nodeSelectedIcon).remove();
                $li.prepend($(`<span class="${this.CLASS.checkClass.nodeSelectedIcon}">${this.options.iconSelected}</span>`));
            }
            $ul.append($li);
        });
    ...
    ZdCascader.prototype.select = function (ids) {
        var self = this;
        var currentId = ids.shift();
        var $node = this.$container.find(`li[data-id="${currentId}"]`);

        if ($node.length) {
            this._nodeClick({ currentTarget: $node[0] });
            if (ids.length > 0) {
                setTimeout(function () {
                    self.select(ids);
                }, 0);
    ...
    ZdCascader.prototype.close = function () {
        if (!this.$container.hasClass(this.CLASS.checkClass.wrapFocus)) return;
        this.$container.removeClass(this.CLASS.checkClass.wrapFocus);
        this.$el.focus();
    }

    ZdCascader.prototype.open = function () {
        if (this.$container.hasClass(this.CLASS.checkClass.wrapFocus)) return;
